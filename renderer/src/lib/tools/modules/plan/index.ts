...
import type { PlanService } from "@/lib/services/plan";
...
The plan status is automatically derived from step statuses unless explicitly set.`,
      ...
      execute: async (args) => {
        ...
        const result = planService.update(planId, updates);
        ...
        const progress = planService.getProgress(planId);
    ...
    defineTool({
      ...
      execute: async (_args, context) => {
        const plan = planService.getBySessionId(context.sessionId);
