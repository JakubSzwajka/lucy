...
export class ChatService {
  ...
  private async prepareChat(agentId: string, userId: string, options: ChatPrepareOptions = {}): Promise<ChatContext | null> {
    ...
    // Inject identity document (synthesized user profile from memories)
    try {
      const identity = await getIdentityService().getActive(userId);
      if (identity?.content) {
        ...
        if (c.keyRelationships?.length) parts.push(`Key relationships: ${c.keyRelationships.map(r => `${r.name} (${r.nature})`).join("; ")}`);
      ...
      // Identity context is non-critical; don't block chat
    }
