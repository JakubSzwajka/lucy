...
export class PageBlock {
  constructor(public readonly obj: Array<PartialBlockObjectResponse | BlockObjectResponse>) {}

  toMarkdown() {
    let markdown = "";
    for (const block of this.obj) {
      markdown += this.blockToMarkdown(block as unknown as Block);
    }
    return markdown;
  }
  ...
  bulletedListItemToMarkdown(block: BulletedListItemBlock) {
    ...
    for (const text of block.bulleted_list_item.rich_text) {
      if (text.type === BlockType.Text) {
        markdown += text.text.content;
      } else if (text.type === BlockType.Mention) {
        markdown += `[page-title](${text.href})`;
      }
    }
    markdown += "\n\n";
    return markdown;
