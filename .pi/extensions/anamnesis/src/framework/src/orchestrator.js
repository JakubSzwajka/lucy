...
export class Orchestrator {
  ...
  /**
   * Check extracted memories for confabulation
   * Verifies source_quote exists in conversation
   * @private
   */
  _checkConfabulation(memories, conversation) {
    ...
    return memories.map(memory => {
      ...
      // Check if source_quote appears in conversation (fuzzy: check first 50 chars)
      const quoteSnippet = memory.source_quote.toLowerCase().slice(0, 50);
      const found = conversationLower.includes(quoteSnippet);

      if (!found) {
        // Try with smaller snippet (first 30 chars) for fuzzy matching
        const smallerSnippet = memory.source_quote.toLowerCase().slice(0, 30);
        const fuzzyFound = conversationLower.includes(smallerSnippet);
