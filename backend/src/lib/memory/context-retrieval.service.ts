...
export class ContextRetrievalService {
  ...
  formatMemoryContext(result: ContextResult): string | null {
    ...
    if (questions.length === 0 && !identity?.content) return null;
    ...
    if (questions.length > 0) {
      const qLines = questions.map((q) => `- [${q.id}] ${q.content}`);
      sections.push(`## Things I've Been Wondering\nThese are questions generated by your background reflection process. They represent gaps in your understanding of Kuba — things that came up in conversation but weren't fully resolved. When the moment feels natural, weave these into conversation. Don't dump them all at once. Don't force it. If Kuba is deep in a technical discussion, don't interrupt with a personal question. But if there's a natural pause, a topic shift, or Kuba explicitly asks "what are you wondering?" — that's your moment.
Once a question is answered, use the continuity tool with action "resolve_question" to mark it resolved — pass the question ID and the answer.
Current questions:\n\n${qLines.join("\n")}`);
    }
